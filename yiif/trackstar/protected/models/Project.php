...
class Project extends TrackStarActiveRecord //extend in order to use override methods, e.g. beforeValidate
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			//array('description, create_time, update_time', 'safe'), //comment out as no need to enter from form
			array('description','safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, description, create_time, create_user_id, update_time, update_user_id', 'safe', 'on'=>'search'),
			array(array('description','name','create_time','create_user_id'), 'required'),
	...
	public function relations()
	{
		...
		return array(
			...
			'users' => array(self::MANY_MANY, 'User', 'tbl_project_user_assignment(project_id, user_id)'), //neccessary to indicate variables
		);
	}
	...
	*/
	public function isUserInRole($role)
	{
		...
		Yii::log('the pid='.$this->id.' user id =='.Yii::app()->user->getId().' role=='.$role,'info','Project.isUserInRole');
		$command = Yii::app()->db->createCommand($sql);
		$command->bindValue(":projectId", $this->id, PDO::PARAM_INT);
	...
	*/
	...
	{
		Yii::log('inside associateUserToRole start==== project id=='.$this->id.' user id=='.$userId.' role==='.$role,'info','Project.associateUserToRole');
